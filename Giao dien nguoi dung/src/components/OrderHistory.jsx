        ...
        <table className="history-table min-w-[480px]">
          ...
          <tbody>
            {displayOrders.map(order => {
              ...
              const urgencyClass = order.urgency === 'Cấp cứu khẩn' ? 'text-red-400 font-semibold' : 'text-ink-soft';
              ...
              return (
                ...
                >
                  <td className="font-mono text-xs font-semibold text-medical-500">
                    #{order.code || `SAH-${String(order.id).padStart(4, '0')}`}
                  </td>
                  <td className="text-sm font-medium text-ink">{order.item || '--'}</td>
